...
define consul_srv::service(
  ...
  file { "${consul_srv::config_dir}/service_${escaped_id}.json":
    ensure  => $ensure,
    owner   => $consul_srv::user_real,
    group   => $consul_srv::group_real,
    mode    => $consul_srv::config_mode,
    content => consul_sorted_json($service_hash, $consul_srv::pretty_config, $consul_srv::pretty_config_indent),
