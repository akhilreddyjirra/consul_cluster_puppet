...
define consul_srv::check(
  ...
  -> file { "${consul_srv::config_dir}/check_${escaped_id}.json":
    ensure  => $ensure,
    owner   => $consul_srv::user_real,
    group   => $consul_srv::group_real,
    mode    => $consul_srv::config_mode,
    content => consul_sorted_json($check_hash, $consul_srv::pretty_config, $consul_srv::pretty_config_indent),
