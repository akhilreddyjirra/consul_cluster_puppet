...
define consul_srv::watch(
  ...
  if (versioncmp($consul_srv::version, '0.4.0') < 0) {
  ...
  -> file { "${consul_srv::config_dir}/watch_${id}.json":
    ensure  => $ensure,
    owner   => $consul_srv::user_real,
    group   => $consul_srv::group_real,
    mode    => $consul_srv::config_mode,
    content => consul_sorted_json($watch_hash, $consul_srv::pretty_config, $consul_srv::pretty_config_indent),
